...
for (iteration in 1:n_iterations) {
  ...
  for (h in 2:224) { 
    ...
    if (!(is.null(psu_selectionnees[[PSU_precedente]])) & psu_counts$PSU_malade[PSU_precedente]) {
      ...
      # Ensuite, on sélectionne la PSU actuelle avec sa probabilité actualisé
      if (runif(n = 1, min = 0, max = 1) < psu_counts$proba_mise_a_jour[PSU_actuelle]) {
        psu_selectionnees[[PSU_actuelle]] <- psu_counts[PSU_actuelle, ]
      }
    }
...
plot_total_individus <- ggplot(resultats_simulation_prop_final, aes(x = as.factor(taux_cluster), y = Total_Individus)) +
  geom_boxplot(fill = "skyblue", color = "black") +
  labs(
...
plot_total_malades <- ggplot(resultats_simulation_prop_final, aes(x = as.factor(taux_cluster), y = Total_Malades)) +
  geom_boxplot(fill = "salmon", color = "black") +
  labs(
