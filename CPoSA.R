...
# Sélection adaptative et séquentiel des autres PSU (problème de la dernière PSU pour la boucle for, à modifier)
for (i in 2:224) { 
  ...
  if (!(is.null(psu_selectionnees[[PSU_précédente]])) & 
      (runif(n = 1, min = 0, max = 1) < psu_counts$proba_initiale[1])) { 
    # si la PSU précédente est sélectionnée et qu'elle est malade (prévalence > 0.5%)
