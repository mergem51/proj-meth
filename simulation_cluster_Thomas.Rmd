...
for (iteration in 1:n_iterations) {
  ...
  V_HT <- sum((psu_counts$malades^2 / psu_counts$proba_initiale^2) / psu_counts$proba_initiale) - (sum(psu_counts$malades / psu_counts$proba_initiale))^2
  CV_HT <- sqrt(V_HT) / Y_HT # coefficient de variation
  
  # Comparaison PHT et HT
  rapport_variances <- V_PHT / V_HT
  biais_relatif <- (Y_PHT - Y_HT) / Y_HT
  ...
  resultats_simulation_posa[iteration, "biais_relatif"] <- biais_relatif
...
for (iteration in 1:n_iterations) {
  ...
  for (h in 2:225) { 
    ...
    if (!(is.null(psu_selectionnees[[PSU_precedente]])) & psu_counts$PSU_malade[PSU_precedente]) {
      ...
      if (h < 225) {
        for (j in (h+1):225) {
          if (runif(n = 1, min = 0, max = 1) < psu_counts$proba_mise_a_jour[PSU_actuelle] & psu_counts$PSU_malade[PSU_actuelle]) {
            psu_counts$proba_mise_a_jour[j] <- psu_counts$proba_mise_a_jour[j] - 
                                               ((1 - psu_counts$proba_mise_a_jour[h]) / (225 - h)) 
          } else {
            psu_counts$proba_mise_a_jour[j] <- psu_counts$proba_mise_a_jour[j] + 
                                               (psu_counts$proba_mise_a_jour[h] / (225 - h)) 
          }
          
          # Vérification que la probabilité mise à jour ne dépasse pas 1
          psu_counts$proba_mise_a_jour[j] <- ifelse(psu_counts$proba_mise_a_jour[j] > 1, 1, psu_counts$proba_mise_a_jour[j])
        }
      ...
      if (h == 225) {
        if (psu_counts$PSU_malade[PSU_actuelle]) {
          psu_counts$proba_mise_a_jour[PSU_actuelle] <- 1  # Cas où S_h * D_h = 1
        } else {
          psu_counts$proba_mise_a_jour[PSU_actuelle] <- psu_counts$proba_mise_a_jour[PSU_actuelle] - 
                                                         ((1 - psu_counts$proba_mise_a_jour[h]) / (225 - h)) 
        }
        
        # Vérification que la probabilité mise à jour ne dépasse pas 1
        psu_counts$proba_mise_a_jour[PSU_actuelle] <- ifelse(psu_counts$proba_mise_a_jour[PSU_actuelle] > 1, 1, psu_counts$proba_mise_a_jour[PSU_actuelle])
