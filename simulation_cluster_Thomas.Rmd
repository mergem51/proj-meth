...
prop_cluster <- 0.01 # Proportion des malades dans les clusters
        ...
        if (runif(n = 1, min = 0, max = 1) < psu_counts$proba_initiale[1]) {
          ...
          for (j in (h+2):225) {
            psu_counts$proba_mise_a_jour[j] <- max(0, min(psu_counts$proba_mise_a_jour[j] - 
                                                   ((1 - psu_counts$proba_mise_a_jour[1]) / (225 - h)), 1))
          }
        } else {
          for (j in (h+1):225) {
            psu_counts$proba_mise_a_jour[j] <- max(0, min(psu_counts$proba_mise_a_jour[j] + 
                                                   ((psu_counts$proba_mise_a_jour[1]) / (225 - h)), 1))
          }
        }
      }
