...
prop_cluster <- 0.5  # Proportion des malades dans les clusters
...
recensement <- data.frame(
  taille_ech = numeric(0),
  valeur_proba = numeric(0)
)
...
for (iteration in 1:n_iterations) {
  ...
  for (h in 2:225) { 
    ...
    # Mise à jour des probabilités des PSU voisines si la PSU précédente est malade
    if (!(is.null(psu_selectionnees[[PSU_precedente]])) & psu_counts$PSU_malade[PSU_precedente] & h < 225) {
      psu_counts$proba_mise_a_jour[PSU_actuelle] <- 1
      psu_selectionnees[[PSU_actuelle]] <- psu_counts[PSU_actuelle, ]
      ...
      if (h < 225) {
        for (j in (h+1):225) {
            psu_counts$proba_mise_a_jour[j] <- psu_counts$proba_mise_a_jour[j] - 
                                               ((1 - psu_counts$proba_mise_a_jour[h - 1]) / (225 - h)) 
        ...
        if (h < 225) {
          for (j in (h+1):225) {
            psu_counts$proba_mise_a_jour[j] <- psu_counts$proba_mise_a_jour[j] + 
                                               (psu_counts$proba_mise_a_jour[h - 1] / (225 - h)) 
      ...
      if (h == 225) {
        # Sélection de la dernière PSU
        if (!(is.null(psu_selectionnees[[PSU_precedente]])) & psu_counts$PSU_malade[PSU_precedente]) {
          psu_counts$proba_mise_a_jour[h] <- 1
          psu_selectionnees[[PSU_actuelle]] <- psu_counts[PSU_actuelle, ]
        } else {
        if (runif(n = 1, min = 0, max = 1) < psu_counts$proba_mise_a_jour[PSU_actuelle]) {
          psu_selectionnees[[PSU_actuelle]] <- psu_counts[PSU_actuelle, ]
    ...
    taille <- length(psu_selectionnees[!sapply(psu_selectionnees, is.null)]) 
    recensement[h, "taille_ech"] <- ifelse(is.null(taille), 0, taille) + 0
    recensement[h, "valeur_proba"] <- psu_counts$proba_mise_a_jour[PSU_actuelle]
  }
