...
```{r}
i = 0
m_min = 27 # Nombre minimal de PSU sélectionnées
n_iterations = 5000
...
resultats_cposa_07$n_min <- "n_min = 07"
resultats_cposa_08$n_min <- "n_min = 08"
resultats_cposa_nwho$n_min <- "n_min = nwho"

resultats_posa_07$n_min <- "n_min = 07"
resultats_posa_08$n_min <- "n_min = 08"
resultats_posa_nwho$n_min <- "n_min = nwho"
...
p1 <- ggplot(resultats, aes(x = as.factor(taux_cluster), y = taille_echantillon, fill = categorie)) +
  ...
  theme(
    ...
    axis.text.x = element_text(size = 10),      # Taille des valeurs de l'axe X
...
resultats_simulation_prop_final_07$n_min <- "n_min = 07"
resultats_simulation_prop_final_08$n_min <- "n_min = 08"
resultats_simulation_prop_final_nmin$n_min <- "n_min = nwho"

# Stratifie l'échantillonnage par taux_cluster en conservant les proportions initiales
resultats_simulation_prop_final_07_01 <- resultats_simulation_prop_final_07 %>%
  group_by(taux_cluster) %>%
  sample_frac(0.1) %>%
  ungroup()

resultats_simulation_prop_final_08_01 <- resultats_simulation_prop_final_08 %>%
  group_by(taux_cluster) %>%
  sample_frac(0.1) %>%
  ungroup()

resultats_simulation_prop_final_nmin_01 <- resultats_simulation_prop_final_nmin %>%
  group_by(taux_cluster) %>%
  sample_frac(0.1) %>%
  ungroup()

# Définition des valeurs de n_min
n_min_values <- c("n_min = 07", "n_min = 08", "n_min = nwho")
...
analyser_n_min <- function(n_min_value) {
  ...
  # Fonction pour créer un graphique avec une personnalisation de l'axe X et Y
  create_plot <- function(data, var, y_label, titre) {
    valeurs_x <- c(0, 0.3, 0.4, 0.5, 0.6, 0.7)  # Valeurs spécifiques de taux_cluster
  
    ggplot(data, aes(x = taux_cluster, y = .data[[var]])) + 
      geom_line(data = data %>% filter(taux_cluster %in% valeurs_x)) +  # Assure que le tracé reste dans l'intervalle
      geom_point() +  
      geom_hline(yintercept = 1, linetype = "dashed", color = "red") +
      labs(x = "Taux Cluster", y = y_label, title = paste(titre, n_min_value)) + 
      theme_minimal() +
      theme(
      ...
      scale_x_continuous(
        breaks = valeurs_x,  # Garde uniquement les 6 valeurs définies
        labels = valeurs_x,
        expand = c(0, 0)
      )
  }
  ...
  plots <- lapply(seq_along(variables_a_visualiser), function(i) {
    var <- variables_a_visualiser[i]
    y_label <- y_labels[i]
    create_plot(comparaison, var, y_label, paste("Graphique de", var))
  })
  
  return(plots)
}

# Exécuter l'analyse pour chaque valeur de n_min et stocker tous les graphiques
all_plots <- lapply(n_min_values, analyser_n_min)

# Affichage des 12 graphiques
...
all_plots[[1]][[4]] <- all_plots[[1]][[4]] + ggtitle("n_min = 0.7")
all_plots[[2]][[4]] <- all_plots[[2]][[4]] + ggtitle("n_min = 0.8") + theme(axis.title.y = element_blank())
all_plots[[3]][[4]] <- all_plots[[3]][[4]] + ggtitle("n_min = nwho") + theme(axis.title.y = element_blank())

grid.arrange(all_plots[[1]][[4]], all_plots[[2]][[4]], all_plots[[3]][[4]], ncol = 3)
