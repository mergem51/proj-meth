...
prop_cluster <- 0.3  # Proportion des malades dans les clusters
mean_radius <- 100    # Rayon moyen des clusters

# Calcul du nombre total de malades
n_malades <- floor(250000 * 0.005)  # 10% de la population sont malades
...
# Pour répartir équitablement 1/3 des malades dans chaque cluster
...
for (i in 1:n_centres) {
  ...
  # Si le nombre d'individus dans le rayon est plus grand que le nombre de malades à attribuer, sélectionner aléatoirement
...
# 2. Attribution des malades restants de manière aléatoire en dehors des clusters
# Liste des indices des malades dans les clusters
indices_malades_clusters <- integer(0)

# Créer une liste pour stocker les informations des individus dans chaque cluster
...
for (i in 1:n_centres) {
  ...
  # Ajouter ces individus à la liste "individus_dans_les_clusters"
  individus_dans_les_clusters[[i]] <- population[individus_dans_cluster, c("id", "x", "y")]

}

individus_dans_les_clusters <- rbind(individus_dans_les_clusters[[1]],individus_dans_les_clusters[[2]],individus_dans_les_clusters[[3]])

# Ecartement des individus dans les cluster
individus_hors_cluster <- unique(population %>% 
                                   filter(!(id %in% unique(individus_dans_les_clusters$id))))

# Attribution du caractére malade dans cette population
...
for (iteration in 1:n_iterations) {
  ...
  for (h in 2:225) { 
    ...
    Sh <- ifelse(!is.null(psu_selectionnees[[PSU_actuelle]]), 1, 0)
...
```{r}
plot_taille_08_nwho_gtrs <- ggplot(resultats_simulation_posa_final, aes(x = as.factor(taux_cluster), y = taille_echantillon)) +
...
plot_malades_08_nwho_gtrs <- ggplot(resultats_simulation_posa_final, aes(x = as.factor(taux_cluster), y = nombre_malades)) +
...
for (iteration in 1:n_iterations) {
  ...
  if (h == 1) {
    ...
    if (runif(n = 1, min = 0, max = 1) < psu_counts$proba_initiale[1]) {
      ...
      if (psu_counts$PSU_malade[1]) {
        psu_selectionnees[[1]] <- psu_counts[1, ]
      psu_counts[2,ncol(psu_counts)] <- 1
      } 
        for (j in (h+2):225) {
            psu_counts$proba_mise_a_jour[j] <- psu_counts$proba_mise_a_jour[j] - 
                                               ((1 - psu_counts$proba_mise_a_jour[1]) / (225 - h)) 
            
          # Vérification que la probabilité mise à jour ne dépasse pas 1 et soit positive
          psu_counts$proba_mise_a_jour[j] <- max(0, min(psu_counts$proba_mise_a_jour[j], 1))
          }
    } else {
        for (j in (h+1):225) {
            psu_counts$proba_mise_a_jour[j] <- psu_counts$proba_mise_a_jour[j] + 
                                               ((psu_counts$proba_mise_a_jour[1]) / (225 - h)) 
  ...
  for (h in 2:224) { 
    ...
    if (runif(n = 1, min = 0, max = 1) < psu_counts$proba_initiale[PSU_precedente]) {
      ...
      if (psu_counts$PSU_malade[PSU_precedente]) {
        ...
        psu_counts[PSU_actuelle,ncol(psu_counts)] <- 1
    
      # Mise à jour des probabilités uniquement si h < 225
      if (h < 225) {
        for (j in (h+2):225) {
            psu_counts$proba_mise_a_jour[j] <- psu_counts$proba_mise_a_jour[j] - 
                                               ((1 - psu_counts$proba_mise_a_jour[PSU_precedente]) / (225 - h)) 
        ...
        if (h < 225) {
          for (j in (h+1):225) {
            psu_counts$proba_mise_a_jour[j] <- psu_counts$proba_mise_a_jour[j] + 
                                               (psu_counts$proba_mise_a_jour[PSU_precedente] / (225 - h)) 
  ...
  # Enregistrement des résultats
  resultats_simulation_cposa[iteration, "taille_echantillon"] <- sum(psu_counts_ech$total_individus)
  resultats_simulation_cposa[iteration, "nombre_malades"] <- sum(psu_counts_ech$malades)
  resultats_simulation_cposa[iteration, "m_ech_malades"] <- m_ech
  resultats_simulation_cposa[iteration, "m_ech"] <- sum(psu_counts$psu_selectionne) 
  }
...
## Simulation Monte-Carlo PoSA non fini
...
for (iteration in 1:n_iterations) {
  ...
  for (h in 2:225) { 
    ...
    if (runif(n = 1, min = 0, max = 1) < psu_counts$proba_initiale[PSU_actuelle]) {
      psu_counts$psu_selectionne[PSU_actuelle] <- 1 
